...
import ir.piana.boot.inquiry.endpoint.data.tables.EndpointLimitation;
import ir.piana.boot.inquiry.endpoint.data.tables.ServicePoint;
import ir.piana.boot.inquiry.endpoint.data.tables.daos.EndpointDao;
import ir.piana.boot.inquiry.endpoint.data.tables.daos.ServicePointDao;
import ir.piana.boot.utils.endpointlimiter.dto.EndpointDto;
import ir.piana.boot.utils.endpointlimiter.dto.EndpointLimitationDto;
import ir.piana.boot.utils.endpointlimiter.dto.ServicePointCollectionDto;
import ir.piana.boot.utils.endpointlimiter.dto.ServicePointDto;
...
public class EndpointDBScheduler extends FixedIntervalScheduler {
    ...
    @Override
    public void run() {
        List<ServicePointDto> servicePoints = new ArrayList<>();
                    ...
                    .fetch(record ->
                            new EndpointDto(
                                    ...
                                    Arrays.stream(Optional.ofNullable(record.get(Endpoint.ENDPOINT.TLS_VERSIONS))
                                            .orElse(TLS.V_1_3.name()).split(",")).toList(), null)
                    );
